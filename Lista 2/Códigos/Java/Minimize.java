/*        Código em java feito com base nos apresentador em aula
*/
...
class LE {
    ...
    public synchronized void EntraLeitor (int id) {
      try { 
        ...
        while ((fila>0) || (this.escr > 0)) { // Caso exista algum thread escritora na fila, os leitores tem sua entrada bloqueada
    ...
    public synchronized void SaiLeitor (int id) {
       this.leit--; //registra que um leitor saiu
        if (this.leit == 0) {
            this.notifyAll();//NotifyAll utilizado para evitar thread starvation
    ...
    public synchronized void EntraEscritor (int id) {
      try { 
        ...
        while ((this.escr > 0) || (this.leit>0)) {
            ...
            fila++; //Variaavel fila criada para que as threads que tentam escrever entrem numa 'fila de prioridade'
                    //Só permitindo que os leitores voltem a ler quando não tiver mais ninguém na fila
            wait();  //bloqueia pela condicao logica da aplicacao 
        }
        this.escr++;
        if(this.escr>0){
    ...
    public synchronized void SaiEscritor (int id) {
       ...
       notify();  //Notify utilizado para evitar que só threads escritoras entrem uma atras da outra
